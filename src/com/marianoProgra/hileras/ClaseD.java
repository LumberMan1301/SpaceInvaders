...
public class ClaseD implements SuperLevel {
    ...
    private ListaCircular<EnemyType> enemies = new ListaCircular<>();
    ...
    public void addEnemies() {
        for (int y = 0; y <1+ GameScreen.getCant_hileras(); y++) {
            for (int x = 0; x < 10; x++) {
                int vida = (int) (Math.floor(Math.random()*20))+1;
                EnemyType e = new Subdito(150 + (x * 40), 35 + (y * 40), 1, 3, vida,1.5d+GameScreen.getSpeed());
                enemies.agregar(e);
