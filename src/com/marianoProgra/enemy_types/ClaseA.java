...
public class ClaseA extends EnemyType{
    ...
    public ClaseA(double xPos, double yPos, int rows, int columns) {
    ...
    public boolean collide(int i, Player player, Lista<EnemyType> enemys) {
        ...
        for(int w = 0; w < player.playerWeapons.weapons.size(); w++) {
            if(enemys != null && player.playerWeapons.weapons.get(w).collisionRect(((ClaseA) enemys.get(i)).getRect())) {
